...
export const NotificationsDialog: React.FC<NotificationsDialogProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-start justify-end p-4">
      <div className="bg-[#152035] rounded-2xl w-full max-w-sm shadow-2xl p-6 mt-16 animate-in slide-in-from-right">
        <div className="flex justify-between items-center mb-6">
          <h2 className="font-bold text-lg text-slate-800 flex items-center gap-2">
            <Bell className="w-5 h-5 text-blue-600" /> Notificaciones
          </h2>
          <div className="flex items-center gap-2">
            <button onClick={() => setShowRead(!showRead)} className={cn("text-xs px-2 py-1 rounded", showRead ? "bg-blue-100 text-blue-800" : "bg-[#111A2E] text-slate-600")}>
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          {filteredNotifications.map((n: any) => (
            <div key={n.id} className={cn("p-4 rounded-lg border", n.read ? "bg-[#0D1527] border-[#1E293B]" : "bg-blue-50 border-blue-100")}>
