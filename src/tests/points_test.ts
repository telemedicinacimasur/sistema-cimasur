...
import { PointsWorkflow, PointsRule } from '../services/automation/definitions';

// Mock DB
let db: any = {
    'automation_workflows': [PointsWorkflow],
    'automation_rules': [PointsRule],
    'workflow_executions': [],
    'automation_jobs': [],
    'automation_history': [],
...
async function runTest() {
    console.log('--- Starting Points Test ---');
    ...
    const clientId = 'client_111';
    
    // 1. Trigger Event
    console.log('Step 1: Triggering points_earned');
    await core.triggerEvent('points_earned', { id: clientId }, 'system');
