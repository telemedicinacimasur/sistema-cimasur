...
import { BirthdayWorkflow, BirthdayRule } from '../services/automation/definitions';

// Mock DB
let db: any = {
    'automation_workflows': [BirthdayWorkflow],
    'automation_rules': [BirthdayRule],
    'workflow_executions': [],
    'automation_jobs': [],
    'automation_history': [],
...
async function runTest() {
    console.log('--- Starting Birthday Notification Test ---');
    ...
    const clientId = 'client_555';
    
    // 1. Trigger Event
    console.log('Step 1: Triggering client_birthday');
    await core.triggerEvent('client_birthday', { id: clientId }, 'system');
