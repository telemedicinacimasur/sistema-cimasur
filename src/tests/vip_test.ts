...
import { VIPWorkflow, VIPRule } from '../services/automation/definitions';

// Mock DB
let db: any = {
    'automation_workflows': [VIPWorkflow],
    'automation_rules': [VIPRule],
    'workflow_executions': [],
    'automation_jobs': [],
    'automation_history': [],
...
async function runTest() {
    console.log('--- Starting VIP Notification Test ---');
    ...
    const clientId = 'client_777';
    
    // 1. Trigger Event
    console.log('Step 1: Triggering client_vip_status_changed');
    await core.triggerEvent('client_vip_status_changed', { id: clientId, isVIP: true }, 'system');
